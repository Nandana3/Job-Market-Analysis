...
# ─────────────────────────────────────────────────────────────────
# FINDING 5: Role Comparison — Does the "Skill Ecosystem" Actually
# Differ by Role?
# For each genuine analyst role type: average skill count, top 5
# skills, and domain-knowledge rate, side by side. This directly
# tests whether different role titles demand genuinely different
# skill bundles, or whether it's the same bundle regardless of title.
# ─────────────────────────────────────────────────────────────────

def finding_5_role_comparison(conn):
    print_header("FINDING 5: Does the Skill Bundle Actually Differ by Role?")
    ...
    print(f"\nComparing roles with at least 20 postings (smaller categories excluded for reliability):\n")

    for role in roles:
        cursor.execute("""
            SELECT COUNT(*) FROM jobs
            WHERE data_quality_flag = 'clean' AND title_normalized = ?
        """, (role,))
        role_total = cursor.fetchone()[0]

        # Average skill count for this role
        cursor.execute("""
            SELECT j.job_id, COUNT(s.skill) as cnt
            FROM jobs j JOIN job_skills s ON j.job_id = s.job_id
            WHERE j.data_quality_flag = 'clean' AND j.title_normalized = ?
            GROUP BY j.job_id
        """, (role,))
        skill_counts = [r[1] for r in cursor.fetchall()]
        ...
        cursor.execute("""
            ...
            WHERE j.data_quality_flag = 'clean' AND j.title_normalized = ?
            GROUP BY s.skill ORDER BY cnt DESC LIMIT 5
        """, (role,))
        top_skills = cursor.fetchall()

        print(f"--- {role} ({role_total} postings) ---")
        print(f"  Average distinct skills: {avg_skills:.1f}")
        print(f"  Top 5 skills:")
        for skill, cnt in top_skills:
            pct = cnt / role_total * 100
            print(f"    {skill:22s} {pct:5.1f}%")
        print()

    # Direct comparison table: does the TOP skill differ by role?
    print("Quick comparison — top skill per role:\n")
    for role in roles:
        cursor.execute("""
            SELECT s.skill, COUNT(DISTINCT s.job_id) as cnt
            FROM job_skills s JOIN jobs j ON j.job_id = s.job_id
            WHERE j.data_quality_flag = 'clean' AND j.title_normalized = ?
            GROUP BY s.skill ORDER BY cnt DESC LIMIT 1
        """, (role,))
        top = cursor.fetchone()
        if top:
            print(f"  {role:22s} -> {top[0]}")
...
if __name__ == "__main__":
    ...
    finding_5_role_comparison(conn)
    conn.close()